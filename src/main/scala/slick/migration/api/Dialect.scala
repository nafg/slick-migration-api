...
class DerbyDialect(driver: DerbyDriver) extends Dialect[DerbyDriver](driver) {
  override def columnSql(ci: ColumnInfo, includePk: Boolean = true): String = {
    def name = quoteIdentifier(ci.name)
    def typ = ci.sqlType
    def default = ci.default.map(" DEFAULT " + _).getOrElse("")
    def notNull = if (ci.notNull) " NOT NULL" else ""
    def pk = if (includePk && ci.isPk) " PRIMARY KEY" else ""
    def autoInc = if (ci.autoInc) " GENERATED BY DEFAULT AS IDENTITY" else ""
    s"$name $typ$default$notNull$pk$autoInc"
  }
  ...
  override def renameIndex(oldName: String, newName: String) =
    s"rename index ${quoteIdentifier(oldName)} to ${quoteIdentifier(newName)}"
}
