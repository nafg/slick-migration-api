...
object TableMigration {
  ...
  object Action {
    ...
    case class RenameColumnTo(originalInfo: ColumnInfo, to: String) extends Reversible(2)
    case class AlterColumnType(info: ColumnInfo) extends Action(3)
    case class AlterColumnDefault(info: ColumnInfo) extends Action(3)
    case class AlterColumnNullable(info: ColumnInfo) extends Action(3)
    case class AddPrimaryKey(info: PrimaryKeyInfo) extends Reversible(4)
    case class AddForeignKey(fk: ForeignKey) extends Reversible(4)
    case class CreateIndex(info: IndexInfo) extends Reversible(5)
    case class RenameIndexTo(originalInfo: IndexInfo, to: String) extends Reversible(5)
    case class DropPrimaryKey(info: PrimaryKeyInfo) extends Reversible(6)
    case class DropForeignKey(fk: ForeignKey) extends Reversible(6)
    case class DropIndex(info: IndexInfo) extends Reversible(7)
    case class RenameColumnFrom(currentInfo: ColumnInfo, from: String) extends Reversible(8)
    case class RenameIndexFrom(currentInfo: IndexInfo, from: String) extends Reversible(8)
    case class RenameTableFrom(from: String) extends Reversible(8)
    case class DropColumnOfName(name: String) extends Action(9)
    case class DropColumn(info: ColumnInfo) extends Reversible(9)
    case object DropTable extends Action(10)
  }
